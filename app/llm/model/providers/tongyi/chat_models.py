...
"""Tongyi chat models."""
"""Fork from langchain_community.chat_models.tongyi"""
...
class ChatTongyi(BaseChatModel):
    ...
    def _chat_generation_from_qwen_resp(
        ...
        if is_last_chunk:
            search_results = dict_get(resp["output"], "search_info.search_results", [])

            if search_results and (
                isinstance(message, AIMessage) or isinstance(message, AIMessageChunk)
            ):
                ref_content = "\n\n  ".join(
                    [
                        f"[^{search_result['index']}]: [{search_result['title'][:32] + '...' if len(search_result['title']) > 32 else search_result['title']}]({search_result['url']})"
                        for search_result in search_results
                    ]
                )
                if isinstance(message.content, list):
                    message.content += [ref_content]
